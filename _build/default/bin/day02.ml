...
let rec parse_rounds (rounds : string list) (valid : bool) (max_red : int)
    (max_green : int) (max_blue : int) =
  match rounds with
  | [] ->
  ...
  | hd :: tl -> (
      ...
      | "red" ->
          ...
          if score > max_red then
            parse_rounds tl (valid && res) score max_green max_blue
          else parse_rounds tl (valid && res) max_red max_green max_blue
      | "green" ->
          let res = score <= green in
          if score > max_green then
            parse_rounds tl (valid && res) max_red score max_blue
          else parse_rounds tl (valid && res) max_red max_green max_blue
      | "blue" ->
          let res = score <= blue in
          if score > max_blue then
            parse_rounds tl (valid && res) max_red max_green score
          else parse_rounds tl (valid && res) max_red max_green max_blue
      | _ ->
...
let parse_game game =
  ...
  let first : string = List.nth_exn first_part 1 in
  let rounds : string list = first |> String.split ~on:',' in
  let round_result, first_red, first_green, first_blue =
    parse_rounds rounds true 0 0 0
  in
